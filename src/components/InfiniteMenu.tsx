...
void main() {
    ...
    vAlpha = smoothstep(-0.3, 1.3, normalize(worldPosition.xyz).z) * 0.9 + 0.1;
    vUvs = aModelUvs;
...
void main() {
    ...
    // Boost colors and add a subtle glow towards the center
    outColor.rgb = pow(outColor.rgb, vec3(0.85)) * 1.2;
    outColor.a = texColor.a * vAlpha;
}
`;
...
class ArcballControl {
  ...
  public update(deltaTime: number, targetFrameDuration = 16): void {
    ...
    if (!this.isPointerDown) {
      // Auto-rotation Earth-like effect with axial tilt (~23.5 degrees)
      const autoRotateQuat = quat.create();
      // Axis: [sin(23.5°), cos(23.5°), 0] ≈ [0.398, 0.917, 0]
      const tiltedAxis: vec3 = [0.398, 0.917, 0];
      quat.setAxisAngle(autoRotateQuat, tiltedAxis, 0.004 * timeScale);
      quat.multiply(this.orientation, autoRotateQuat, this.orientation);
    }
...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    // Background for the atlas
    ctx.clearRect(0, 0, canvas.width, canvas.height);

    const drawItem = (item: MenuItem, i: number) => {
      const x = (i % this.atlasSize) * cellSize;
      const y = Math.floor(i / this.atlasSize) * cellSize;
      
      // Draw a colored circular background for the disc
      ...
      // Determine domain for logo fetching
      const domainItem = item.link ? new URL(item.link).hostname : '';
      
      // Try to load image
      const img = new Image();
      img.crossOrigin = 'anonymous';
      
      const drawInitialFallback = () => {
        ctx.save();
        ctx.beginPath();
        // Clear previous drawing attempt if any in this cell
        ctx.clearRect(x, y, cellSize, cellSize);
        
        // Redraw colored circle
        ctx.beginPath();
        ctx.arc(x + cellSize / 2, y + cellSize / 2, cellSize / 2 - 10, 0, Math.PI * 2);
        ctx.fillStyle = color;
        ctx.globalAlpha = 0.8;
        ...
        ctx.fillStyle = 'white';
        ctx.font = `bold ${cellSize * 0.35}px Inter, sans-serif`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        const initials = item.title ? (item.title.length > 1 ? item.title.substring(0, 2).toUpperCase() : item.title[0].toUpperCase()) : '?';
        ctx.fillText(initials, x + cellSize / 2, y + cellSize / 2);
        ctx.restore();
      ...
      img.onload = () => {
        ctx.save();
        ctx.beginPath();
        ctx.arc(x + cellSize / 2, y + cellSize / 2, cellSize / 2 - 30, 0, Math.PI * 2);
        ctx.clip();
        ctx.fillStyle = 'white';
        ctx.fill();
        ctx.drawImage(img, x + 40, y + 40, cellSize - 80, cellSize - 80);
        ctx.restore();
      ...
      img.onerror = () => {
        if (img.src.includes('clearbit')) {
          img.src = `https://logo.horse/icon/${domainItem}`;
        } else if (img.src.includes('logo.horse')) {
          img.src = `https://www.google.com/s2/favicons?domain=${domainItem}&sz=128`;
        } else {
          drawInitialFallback();
        }
      };
      ...
      if (domainItem) {
        img.src = `https://logo.clearbit.com/${domainItem}`;
      } else {
        drawInitialFallback();
      }
    };

    this.items.forEach((item, i) => drawItem(item, i));
    
    // Initial upload of empty colored circles
    gl.bindTexture(gl.TEXTURE_2D, this.tex);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, canvas);
    gl.generateMipmap(gl.TEXTURE_2D);
  }
  ...
  private animate(deltaTime: number): void {
    ...
    positions.forEach((p, ndx) => {
      // Depth scaling: front is large, back is small but still visible
      const s = ((p[2] + this.SPHERE_RADIUS) / (this.SPHERE_RADIUS * 2)) * SCALE_INTENSITY + (1 - SCALE_INTENSITY);
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center pointer-events-none">
      <canvas
        ...
        ref={canvasRef}
        className="cursor-grab w-full h-full overflow-hidden absolute inset-0 outline-none active:cursor-grabbing pointer-events-auto"
      />

      {activeItem && (
        <div className="relative z-20 flex flex-col items-center gap-4 text-center px-4 max-w-4xl">
          <h2
            className={`
          select-none
          font-black
          text-[3.5rem] md:text-[7rem]
          text-[#1e3a5f] drop-shadow-[0_0_50px_rgba(255,255,255,1)]
        ...
        `}
          ...
          <p
            className={`
          select-none
          text-[1.8rem] md:text-[2.8rem]
          text-[#1e3a5f] font-bold drop-shadow-[0_0_30px_rgba(255,255,255,1)]
